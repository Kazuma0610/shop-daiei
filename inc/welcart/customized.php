			...
			<?php else : ?>
				...
				<div class="review-info">
					<div class="review-author vcard">
						<?php printf( wp_kses_post( '%s <span>says</span>', 'welcart_mode' ), sprintf( '<cite class="fn">%s</cite>', get_comment_author_link() ) ); ?>
...
function welcart_mode_after_login_redirect() {
	if ( isset( $_REQUEST['login_ref'] ) && ! empty( $_REQUEST['login_ref'] ) ) {
		wp_redirect( esc_url( $_REQUEST['login_ref'] . '#wc_reviews' ) ); //phpcs:ignore
		exit;
	}
}
 ...
 * @return void
 */
function welcart_mode_single_item_autodelivery_sku_select() {
	global $post, $usces;
